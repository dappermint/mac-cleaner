...
**actions** is the cleanup list, sorted by macOS storage category. Four risk levels: `safe` has a supported command behind it, `review` needs a human look, `destructive` needs an exact confirmation phrase, `protected` is visible but never selectable. Your Steam library does not become "clutter" because it is large.
